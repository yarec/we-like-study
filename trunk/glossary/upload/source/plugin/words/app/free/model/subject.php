...
class m_subject extends wls implements dbtable,fileLoad{
	...
	public function delete($ids){
		...
		mysql_query($sql,$conn);
		$sql = "delete from ".$pfx."wls_user_group2subject where id_level_subject like '".$temp['id_level']."%'";
		mysql_query($sql,$conn);		
		return true;
	}
	...
	public function create(){
		...
		$sql = "drop table if exists pre_wls_subject;";
		$sql = str_replace("pre_", $pfx, $sql);
		mysql_query($sql,$conn);
		$sql = "
			create table pre_wls_subject(
				...
				,count_glossary int default 0
				
				,questiontypes varchar(200) default '0'
		...
		mysql_query($sql,$conn);
			...
			$sql = "drop table if exists pre_wls_user_group2subject;";
			$sql = str_replace("pre_", $pfx, $sql);
			mysql_query($sql,$conn);
			$sql = "
	...
	public function importAll($path){
		if($this->phpexcel==null){
			...
			$currentSheet = $this->phpexcel->getSheetByName($this->il8n['user']['subject']);
		...
		for($i='A';$i<=$allColmun;$i++){
			...
			if($currentSheet->getCell($i.$keysRow)->getValue()==$this->il8n['user']['isknowledge']){
				$keys['isknowledge'] = $i;
			}
			if($currentSheet->getCell($i.$keysRow)->getValue()==$this->il8n['user']['ordering']){
				$keys['ordering'] = $i;
			}
		}
	...
	public function exportAll($path=null){
		...
		$objPHPExcel->getActiveSheet()->setTitle($this->il8n['user']['subject']);
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A2', $this->il8n['user']['id_level']);
		...
		$objPHPExcel->getActiveSheet()->setCellValue('D2', $this->il8n['user']['ids_level_knowledge']);
		$objPHPExcel->getActiveSheet()->setCellValue('E2', $this->il8n['user']['icon']);
		$objPHPExcel->getActiveSheet()->setCellValue('F2', $this->il8n['user']['description']);
	...
	public function getQuestionTypes($id_level){
		$pfx = $this->cfg->dbprefix;
		$conn = $this->conn();
		
		$sql = "select questiontypes from ".$pfx."wls_subject where id_level = ".$id_level;
		$res = mysql_query($sql,$conn);
		$temp = mysql_fetch_assoc($res);
		return $temp['questiontypes'];
	}
}
?>
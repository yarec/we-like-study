...
wls.question.choice = Ext.extend(wls.question, {
	...
	marking:function(){
		...
		if(this.markingmethod!=0){
			...
			$(".w_qw_tool", $('#w_qs_' + this.id)).append("<a href='#' onclick='wls_question_marking("+this.id+","+this.cent+","+this.id+")'>"+il8n.marking+"</a>");
		}
	},
	...
	showDescription : function() {
		...
		if(obj.markingmethod==1){
			...
			if(this.quiz.type=='log'){
				$(".w_qw_tool", $('#w_qs_' + this.id)).append("<a href='#' onclick='wls_question_markked("+this.quiz.logData.id+","+this.id+")'>"+il8n.marking+"</a>");
			}
		}else{			
			//Give Up

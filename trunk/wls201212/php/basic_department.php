<?php
class basic_department {
    
    public function loadConfig($return='json') {
        $CONN = tools::conn();
        $config = array();
        
        $sql = "select code,value from basic_parameter where reference = 'basic_department__type' order by code";
	...
	public function getGrid(){
		...
		$sql = "select * from basic_department order by code;";
		$res = mysql_query($sql,$CONN);
	...
	public function importExcel($path=NULL){
		...
		$CONN = tools::conn();
		$currentSheet = $obj->getSheetByName($il8n['basic_department']['basic_department']);
		if($currentSheet==null){
			//如果这个sheet页不存在,就报错
			tools::error(array("state"=>0,"msg"=>$il8n['requestSheetMissing']." : ".$il8n['basic_department']['basic_department']),'json');
		}	
		...
		if($currentSheet->getCell("C1")->getValue()!=$il8n['basic_department']['code']){
            tools::error(array("state"=>0,"msg"=>'C1 wrong header format'),'json');
        }     
		if($currentSheet->getCell("D1")->getValue()!=$il8n['basic_department']['icon']){
            tools::error(array("state"=>0,"msg"=>'D1 wrong header format'),'json');
        }     
		if($currentSheet->getCell("E1")->getValue()!=$il8n['basic_department']['remark']){
            tools::error(array("state"=>0,"msg"=>'E1 wrong header format'),'json');
        }                          
        
        $allRow = $currentSheet->getHighestRow();
	    $types_ = self::getTypes();
        $types = array();
        for($i=0;$i<count($types_);$i++){
            $types[$types_[$i]['value']] = $types_[$i]['code'];
        }
        for($i=2;$i<=$allRow;$i++){
            $sql = "select * from basic_department where code = '".$currentSheet->getCell("C".$i)->getValue()."' ";
            $res = mysql_query($sql,$CONN);
            ...
            $sql = "insert into basic_department (name,type,code,icon,remark) values (
            ...
            if($types[$currentSheet->getCell("B".$i)->getValue()]=='2'){
                $sql = "insert into basic_department (code , name , remark ) values ( '".$currentSheet->getCell("C".$i)->getValue()."'
                ,'".$currentSheet->getCell("A".$i)->getValue()."'
                ,'Excel Import' ) ";
                 mysql_query($sql,$CONN);
            }
        ...
        mysql_query("call basic_department_setleaf();",$CONN);
	...
	public function insert(){
		...
		$sql = "select * from basic_department where code = '".$json['code']."'; ";
		$res = mysql_query($sql,$CONN);
		if( !mysql_fetch_assoc($res) ){    
    		...
    		$sql2 = "insert into basic_department (".$keys.") values ('".$values."')";		
    ...
    public function view(){
        $CONN = tools::conn();    
        $code = $_REQUEST['code'];
        $res = mysql_query( "select * from basic_department where code = '".$code."' " , $CONN );
	...
	public function downloadAll(){
		...
		$objPHPExcel->getActiveSheet()->setTitle( $il8n['basic_department']['basic_department'] );
		$objPHPExcel->getActiveSheet()->setCellValue('A1', $il8n['title'] );
		$objPHPExcel->getActiveSheet()->setCellValue('B1', $il8n['type'] );
		$objPHPExcel->getActiveSheet()->setCellValue('C1', $il8n['basic_department']['code'] );
		$objPHPExcel->getActiveSheet()->setCellValue('D1', $il8n['basic_department']['icon'] );
		$objPHPExcel->getActiveSheet()->setCellValue('F1', $il8n['basic_department']['remark'] );
		
		$sql = "select * from basic_department ;";
		$res = mysql_query($sql,$CONN);
		$data = array();
		while($temp = mysql_fetch_assoc($res)){
			$data[] = $temp;
		...
		$file = "basic_department_".date('YmdHis').".xls";
    ...
    public function update($data=NULL,$retur='json') {
        ...
        $sql = "update basic_department set ";

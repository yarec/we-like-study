...
CREATE PROCEDURE `basic_user__logout`(IN `in_username` varchar(200),IN `in_session` varchar(200), OUT out_msg varchar(200) , out out_state int)
BEGIN
    declare fig int;
    declare username_ varchar(200);        
    declare id_user_ int;
...
CREATE PROCEDURE `basic_workflow__check`(IN `in_username` varchar(200),IN `in_session` varchar(200),OUT `out_msg` varchar(200),OUT `out_state` varchar(200),OUT `out_session` varchar(200),OUT `out_app` varchar(200))
BEGIN	
    declare username_ varchar(200);        
    declare password_ varchar(200);
    declare session_ varchar(200);        
...
pro_main:BEGIN
    ...
    call education_paper__import(in_guid,@p_id,@p_state,@p_msg); 
    if @p_state <> 1 then    
        set out_state = 2;        
        set out_msg = @p_msg;        
        leave pro_main;
...
CREATE PROCEDURE `education_paper__import`(IN in_guid char(36),out out_id int,OUT out_state int,OUT out_msg varchar(200))
pro_main:BEGIN
    declare fig int;     
    declare A_,B_,C_,D_,E_,F_,G_,H_,I_,J_,K_,L_,M_ varchar(200);    
    declare A__,B__,C__,D__,E__,F__,G__,H__,I__,J__,K__,L__,M__ varchar(2000);            
    declare id_creater_,id_creater_group_ int default 0;                
    declare code_,row1_,row2_ varchar(300);
    declare cur_array cursor for     
    ...
    set @columnsimport = 'title,subject,cost,author,remark';    
    set @sql_keys_paper = '';    
    ...
    while( fig = 0 ) do      
        ...
        elseif row1_ = 'subject' then 
            set @temp = null;
            select code into @temp from education_subject where code = row2_;                
            if @temp is null then             
        ...
        elseif row1_ = 'author' then 
            set @temp = null;
            select id,id_group into @temp,@temp2 from basic_user where username = row2_;                
            #select @temp;
            if @temp is null then             
            ...
            set @id_creater_ = @temp;              
            set @id_creater_group_ = @temp2;                             
    ...
    call education_question__import(in_guid,@q_state,@q_msg,@q_ids,@q_cent,@q_count);   
    if @q_state = 0 then        
        set out_state = 2;        
        set out_msg = @q_msg;      
        delete from basic_excel where guid = in_guid;          
    ...
    set @sql_insert_paper = concat("insert into education_paper ("
                             ,@sql_keys_paper
                             ," cent,id_creater,id_creater_group,id,count_questions) values ( "
                             ,@sql_values_paper
                             ,"'",@q_cent,"',"
                             ,"'",id_creater_,"',"
                             ,"'",id_creater_group_,"',"
...
CREATE PROCEDURE `education_student__import`(IN in_guid char(36),OUT out_state int,OUT out_msg varchar(200),OUT out_ids varchar(2000))
pro_main:BEGIN
/*
...
*/
    ...
    declare cur_student cursor for
        ...
        where guid = in_guid 
        and rowindex > 2
        and sheetname = basic_memory__il8n('student','education_student',1)
        order by rowindex;     
    declare CONTINUE HANDLER FOR SQLSTATE '02000' SET fig = 1;     

    #得到EXCEL中,业务数据的总行数                    
    select max(rowindex) into @maxrow from basic_excel where guid = in_guid and sheetname = basic_memory__il8n('student','education_student',1);             
        ...
        inerLoop: LOOP
            ...
            if @alphaindex >=  @divideColumn then                   
                ...
                if @keyindex = 'attitude_life' then                                 
                    set @temp = null;
                    select code into @temp from basic_memory where extend5 = 'education_student__alife' and extend4 = @tempvalue;            
                    #select @temp;
                    if @temp is null then 
                        set out_state = 0;
                        set out_msg = concat(@tempvalue," ",@alphaindex,rowindex__," ",basic_memory__il8n('cellWrong','basic_excel',1));                        
                ...
                if @keyindex = 'attitude_learn' then                                 
                    set @temp = null;
                    select code into @temp from basic_memory where extend5 = 'education_student__alearn' and extend4 = @tempvalue;            
                    #select @temp;
                    if @temp is null then 
                        set out_state = 0;
                        set out_msg = concat(@tempvalue," ",@alphaindex,rowindex__," ",basic_memory__il8n('cellWrong','basic_excel',1));                        
                ...
                if @keyindex = 'attitude_teacher' then                                 
                    set @temp = null;
                    select code into @temp from basic_memory where extend5 = 'education_student__ateacher' and extend4 = @tempvalue;            
                    #select @temp;
                    if @temp is null then 
                        set out_state = 0;
                        set out_msg = concat(@tempvalue," ",@alphaindex,rowindex__," ",basic_memory__il8n('cellWrong','basic_excel',1));                        
                ...
                if @keyindex = 'attitude_classmate' then                                 
                    set @temp = null;
                    select code into @temp from basic_memory where extend5 = 'education_student__aclassmate' and extend4 = @tempvalue;            
                    #select @temp;
                    if @temp is null then 
                        set out_state = 0;
                        set out_msg = concat(@tempvalue," ",@alphaindex,rowindex__," ",basic_memory__il8n('cellWrong','basic_excel',1));                        
                ...
                if @keyindex = 'attitude_oppositesex' then                                 
                    set @temp = null;
                    select code into @temp from basic_memory where extend5 = 'education_student__aoppositesex' and extend4 = @tempvalue;            
                    #select @temp;
                    if @temp is null then 
                        set out_state = 0;
                        set out_msg = concat(@tempvalue," ",@alphaindex,rowindex__," ",basic_memory__il8n('cellWrong','basic_excel',1));                        
...
##select @student_sql_insert,@person_sql_insert,@user_sql_insert,@user2group_sql_insert;

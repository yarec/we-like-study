...
class m_user_group extends wls implements dbtable,fileLoad{
	...
	public function create($table=null){
		...
		if($table==null||$table=='wls_user_group2subject2teacher'){
	...
	public function importExcelWithAccess($path){
		...
		if($this->c->cmstype==''){
			...
			$data = array(
				...
				$accesssData[] = $data ;
				$accessObj->insert($data);
		}else if($this->c->cmstype=='Joomla' || $this->c->cmstype=='Joomla16'){
			$data = array(
				'name'=>'Joomla',
				'id_level'=>'90',
				'ismenu'=>1,
				...
				$accesssData[] = $data ;
				$accessObj->insert($data);
		}else if($this->c->cmstype=='PhpWind'){
			$data = array(
				'name'=>'PhpWind',
				'id_level'=>'90',
				'ismenu'=>1,
		...
		for($i=4;$i<=$allRow;$i++){
			for($i2=$grouppoint;$i2<=$allColmun;$i2++){
				if($currentSheet->getCell($i2.$i)->getValue()=='√'){
					...
					if($comments!=''){
//						echo $comments;
//						$comments = str_replace($this->lang['instructor'].":","",$comments);

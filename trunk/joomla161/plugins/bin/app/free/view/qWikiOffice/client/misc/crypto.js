...
Ext.ux.Crypto.AES = function() {
	...
	var mixColumns = function(s, Nb) {
		for (var c = 0; c < 4; c++) {
			var a = new Array(4); // 'a' is a copy of the current column from
									// 's'
	...
	var addRoundKey = function(state, w, rnd, Nb) { // xor Round Key into state
													// S [�5.1.4]
		for (var r = 0; r < 4; r++) {
			for (var c = 0; c < Nb; c++)
	...
	var Sbox = [0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01,
			...
			0x8e, 0x94, 0x9b, 0x1e, 0x87, 0xe9, 0xce, 0x55, 0x28, 0xdf, 0x8c,
			0xa1, 0x89, 0x0d, 0xbf, 0xe6, 0x42, 0x68, 0x41, 0x99, 0x2d, 0x0f,
			0xb0, 0x54, 0xbb, 0x16];
	...
	}; // \xa0 to cater for bug in Firefox; include '-' to leave it free for
		// use as a block marker
	...
	return {
		...
		cipher : function(input, w) {
			var Nb = 4; // block size (in words): no of columns in state (fixed
						// at 4 for AES)
			var Nr = w.length / Nb - 1; // no of rounds: 10/12/14 for
		...
		keyExpansion : function(key) {
			...
			var Nk = key.length / 4 // key length (in words): 4/6/8 for
									// 128/192/256-bit keys
			var Nr = Nk + 6; // no of rounds: 10/12/14 for 128/192/256-bit
								// keys
			...
			for (var i = 0; i < Nk; i++) {
				var r = [key[4 * i], key[4 * i + 1], key[4 * i + 2],
						key[4 * i + 3]];
				w[i] = r;
			}
		 ...
		 * http://csrc.nist.gov/publications/nistpubs/800-38a/sp800-38a.pdf for
		 * each block - outputblock = cipher(counter, key) - cipherblock =
		...
		encrypt : function(plaintext, password, nBits) {
			...
			// for this example script, generate the key by applying cipher to
			// 1st 16/24/32 chars of password;
			// for real-world applications, a more secure approach would be to
			// hash the password e.g. with SHA-1
			var nBytes = nBits / 8; // no bytes in key
			var pwBytes = new Array(nBytes);
			for (var i = 0; i < nBytes; i++)
				pwBytes[i] = password.charCodeAt(i) & 0xff;
			var key = this.cipher(pwBytes, this.keyExpansion(pwBytes));
			key = key.concat(key.slice(0, nBytes - 16)); // key is now
															// 16/24/32 bytes
															// long
			...
			var blockSize = 16; // block size fixed at 16 bytes / 128 bits
								// (Nb=4) for AES
			var counterBlock = new Array(blockSize); // block size fixed at
														// 16 bytes / 128 bits
														// (Nb=4) for AES
			var nonce = (new Date()).getTime(); // milliseconds since 1-Jan-1970

			// encode nonce in two stages to cater for JavaScript 32-bit limit
			// on bitwise ops
			for (var i = 0; i < 4; i++)
			...
			var ciphertext = new Array(blockCount); // ciphertext as array of
													// strings
			...
			for (var b = 0; b < blockCount; b++) {
				...
				var cipherCntr = this.cipher(counterBlock, keySchedule); // --
																			// encrypt
																			// counter
																			// block
																			// --
				...
				ciphertext[b] = escCtrlChars(ct); // escape troublesome
													// characters in ciphertext
			}

			// convert the nonce to a string to go on the front of the
			// ciphertext
			var ctrTxt = '';
			for (var i = 0; i < 8; i++)
			...
			// use '-' to separate blocks, use Array.join to concatenate arrays
			// of strings for efficiency
		...
		decrypt : function(ciphertext, password, nBits) {
			...
			ciphertext = ciphertext.split('-'); // split ciphertext into array
			...
			for (var b = 1; b < ciphertext.length; b++) {
				...
				for (var c = 0; c < 4; c++)
					counterBlock[15 - c - 4] = ((b / 0x100000000 - 1) >>> c * 8)
							& 0xff;

				var cipherCntr = this.cipher(counterBlock, keySchedule); // encrypt
																			// counter
																			// block
				...
				plaintext[b - 1] = pt; // b-1 'cos no initial nonce block in
										// plaintext
			}

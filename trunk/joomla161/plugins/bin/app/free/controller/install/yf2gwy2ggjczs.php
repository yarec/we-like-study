...
class install_yf2gwy2ggjczs extends wls {
	...
	public $id_level_subject = '500102';
	function install_yf2gwy2ggjczs(){
		parent::wls();
		$this->types = array(
			'500102'=> array(
				'su_id'=>5
...
var down = function(){
	$.ajax({
		type: 'post',
		url: 'wls.php?controller=install_yf2gwy2ggjczs&action=down',
		data: {id:ids[index]},
	...
	public function getCheckQuestions(){
		...
		for($i=($startPoint+1);$i<=($endPoint+1);$i++){
			...
			$question = array(
				...
				,'index' =>"10".( ($i>9)?$i:('0'.$i) )
				,'ids_level_knowledge'=>$this->ids_level_knowledge[rand(0,4)]
			);
...
var down = function(){
	$.ajax({
		type: 'post',
		url: 'wls.php?controller=install_yf2gwy2ggjczs&action=testLog',

...
class m_integration_Joomla16 extends m_integration implements integrate{
...
//		echo $key;exit();
		...
		$sql = "select username,guest,userid,group_id from 
			...
			where session_id = '".$_COOKIE[$key]."' 
				and ".$pfx."user_usergroup_map.user_id = ".$pfx."session.userid
			";
//		echo $sql;
		...
		if($res2==false || (mysql_fetch_assoc($res2)==false) ){//这个用户的信息还没有同步过来,需要实施数据插入
			...
			if($temp['group_id']==8)$data['id_level_group'] = 10;
	...
	/**
	 * Only when the user jumpped from Joomla16 to wls
	 *

...
class m_subject_log extends wls implements dbtable,log{
	...
	public function insert($data){
		...
		$sql = "insert into ".$pfx."wls_subject_log (".$keys.") values ('".$values."')";

		$done = mysql_query($sql,$conn);
		if($done===false){
			if(isset($data['count_right']) && $data['count_right']==1){
				$sql2 = "update ".$pfx."wls_subject_log set count_right = count_right +1 where
				 date_created = '".$data['date_created']."' and
				 date_slide = '".$data['date_slide']."' and
				 id_level_knowledge = '".$data['id_level_knowledge']."'
				"; 
			}else{
				$sql2 = "update ".$pfx."wls_subject_log set count_wrong = count_wrong +1 where
				 date_created = '".$data['date_created']."' and
				 date_slide = '".$data['date_slide']."' and
				 id_level_knowledge = '".$data['id_level_knowledge']."'
				"; 
	...
	public function update($data){
		...
		$sql = "update ".$pfx."wls_subject_log set ";
	...
	public function create(){
		...
		$sql = "drop table if exists ".$pfx."wls_subject_log;";
		mysql_query($sql,$conn);
		$sql = "
			create table ".$pfx."wls_subject_log(
	...
	public function getList($page=null,$pagesize=null,$search=null,$orderby=null,$columns="*"){
		...
		if($search!=null){
			...
			for($i=0;$i<count($keys);$i++){
				...
				if($keys[$i]=='lastDate'){
					$sql = "select max(date_created) as date_created_max from ".$pfx."wls_subject_log where id_user = ".$search['id_user'];
		...
		$sql = "select ".$columns.",((count_right*100)/(count_right+count_wrong)) as proportion from ".$pfx."wls_subject_log ".$where." ".$orderby;
		...
		$sql2 = "select count(*) as total from ".$pfx."wls_subject_log ".$where;
	...
	public function addLog($whatHappened){
		...
		$id_question = $whatHappened['id_question'];
		
		$data = array(
			 'count_right'=>0
			,'count_wrong'=>0
		);
		if($whatHappened['result']=='right'){
			$data['count_right']=1;
		}else{
			$data['count_wrong']=1;
		}
		if(isset($whatHappened['id_user'])){
			$data['id_user'] = $whatHappened['id_user'];
			$data['ids_level_user_group'] = $whatHappened['ids_level_user_group'];			
		}
		if(isset($whatHappened['date_created'])){
			$data['date_created'] = $whatHappened['date_created'];
		}
		
		$sql = "select ids_level_knowledge from ".$pfx."wls_question where id = ".$id_question;
		$res = mysql_query($sql,$conn);
		$temp = mysql_fetch_assoc($res);
...
left join pre_wls_subject_log 
ON pre_wls_subject_log.id_level_knowledge = pre_wls_subject_knowledge.id_level
where 
pre_wls_subject_log.id 
in (
SELECT 
max(pre_wls_subject_log.id) as max_
...
and pre_wls_subject_log.id_user = ".$user['id']."
GROUP BY pre_wls_subject_knowledge.id_level
	...
	public function getMyRecent($id){
		...
		$sql = "select id_level_knowledge from ".$pfx."wls_subject_log where id_user = ".$user['id']." and id_level_knowledge like '".$id."__' group by id_level_knowledge ";
		...
		while ($temp = mysql_fetch_assoc($res)) {
			...
			$sql_ = "select * from ".$pfx."wls_subject_log where id_user= ".$user['id']." and id_level_knowledge = '".$temp['id_level_knowledge']."' order by date_created desc  ";

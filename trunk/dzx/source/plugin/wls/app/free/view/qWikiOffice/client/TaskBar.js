...
Ext.ux.TaskBar = Ext.extend(Ext.Container, {
	/**
	 * @cfg {Boolean} This feature is not finished!!!
	 */
	autoHide : false,
	/**
	 * @cfg {String}
	...
	constructor : function(config) {
		...
		if (this.autoHide) {
			this.ghostEl = Ext.getBody().createChild({
						tag : 'div',
						cls : 'ux-ghost-taskbar ux-ghost-taskbar-'
								+ this.position
					});
		}
		...
		if (this.autoHide) {
			this.el.on('mouseout', function(e, t, o) {
						if (this.autoHide && !this.startButton.menu.isVisible()) {
							...
							var hide = this.position === 'bottom'
									? y < this.desktop.getViewHeight()
									: y >= this.getHeight();
							if (hide) {
								this.hide();
								this.ghostEl.show();
	...
	addQuickStartButton : function(config) {
		return this.quickStartPanel.add(Ext.apply(config, {
					scale : this.buttonScale
				}));
	},
...
Ext.ux.TaskButtonsPanel = Ext.extend(Ext.BoxComponent, {
	...
	updateScrollButtons : function() {
		...
		this.scrollRight[pos >= parseInt(this.getScrollWidth()
				- this.getScrollArea()) ? 'addClass' : 'removeClass']('ux-taskbuttons-scroller-right-disabled');
	}
});
...
Ext.ux.QuickStartPanel = Ext.extend(Ext.BoxComponent, {
			...
			onRender : function(ct, position) {
				Ext.ux.QuickStartPanel.superclass.onRender.call(this, ct,
						position);

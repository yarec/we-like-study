...
// Example Usage: plugins: [ new Ext.ux.GhostBar({ items: [{ text: 'Click Me'
// }], position: 'top' }) ],

Ext.override(Ext.lib.Region, {
			/**
			 * Returns the shortest distance between this Region and another
			 * Region. Either or both Regions may be Points.
			 * 
			 * @param {Region}
			 *            r The other Region
			 * @return {Number} The shortest distance in pixels between the two
			 *         Regions.
			 */
			getDistanceBetween : function(r) {
				...
				return Math.round(Math.sqrt(Math.pow(this.top - r.bottom, 2)
						+ Math.pow(this.left - r.right, 2)));
			}
		});

/**
 * @class Ext.ux.GhostBar
 * @extends Ext.Toolbar A Toolbar class which attaches as a plugin to any
 *          BoxComponent, and fades in at the configured position whenever the
 *          mouse is brought within a configurable threshold. eg: <code><pre>
 * new Ext.Panel({
...
Ext.ux.GhostBar = Ext.extend(Ext.Toolbar, {
	...
	/**
	 * @cfg {Number} threshold The number of pixels around the toolbar position
	 *      in which fading is performed.
	 */
	threshold : 100,

	/**
	 * @cfg {String} position The alignment of this Toolbar,
	 *      <code><b>top</b></code>, or <code><b>bottom</b></code>.
	 *      Defaults to <code><b>bottom</b></code>.
	 */
	/**
	 * @cfg {Array} offsets A two element Array containing the [x, y] offset
	 *      from the default position in which to display the Toolbar.
	 */

	initComponent : function() {

		// Only use one mousemove listener. Check the cache of GhostBars for
		// proximity on each firing
		if (!this.listenerAdded) {
			Ext.getDoc().on('mousemove',
					Ext.ux.GhostBar.prototype.onDocMouseMove,
					Ext.ux.GhostBar.prototype);
	...
	onClientRender : function() {
		this.clientEl = this.ownerCt.getLayoutTarget ? this.ownerCt
				.getLayoutTarget() : this.ownerCt.el;
	},
	...
	syncPosition : function() {
		...
		this.el.alignTo(this.clientEl, (this.position == 'top')
						? 'tl-tl'
						: 'bl-bl', offsets);
	...
	checkMousePosition : function(e) {
		...
		if (d > this.threshold) {
			...
			// Mouse is within range of this Toolbar, so show it if its not
			// already visible
			if (!this.isVisible()) {
				this.show();
			}

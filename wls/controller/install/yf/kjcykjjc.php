 ...
 * 公共英语2级
 * */
class install_yf_kjcykjjc extends wls {
	...
	public function readList(){
		$content = file("http://www.yfzxmn.cn/com/left/left.jsp?so_id=85&su_id=6");  
		$content = implode("\n", $content);
		$content = mb_convert_encoding($content,'UTF-8','GBK');
		$content = str_replace("\n","",$content);
		
		$fileName="file/yf/会计_会计从业_会计基础.list";
		...
		$fileName="file/yf/会计_会计从业_会计基础.downlist";
		$fileName = mb_convert_encoding($fileName,'GBK','UTF-8');
		$handle=fopen($fileName,"a");
		$ids = '';
		...
		$fileName="file/yf/会计_会计从业_会计基础.ids";
		$fileName = mb_convert_encoding($fileName,'GBK','UTF-8');
		$handle=fopen($fileName,"a");
		fwrite($handle,$ids);
		fclose($handle);		
...
var readPaper = function(){
	...
	$.ajax({
		url: \"wls.php?controller=install_yf_kjcykjjc&action=downLoadPaper2&id=\"+arr[index],
...
<div id=\"console\"><div>
	...
	public function downLoadPaper2(){
		$fileName="file/yf/经济类/模拟题试卷/会计从业资格/会计基础/6_".$_REQUEST['id'].".html";
		$fileName = mb_convert_encoding($fileName,'GBK','UTF-8');
...
//			$data = mb_convert_encoding($data,'UTF-8','GBK');
	...
	public function readContent($path=null){
		...
		if($path==null)$path = "E:/Projects/WEBS/PHP/Discuz_7_2/upload/plugins/wls/file/yf/经济类/模拟题试卷/会计从业资格/会计基础/6_".$_REQUEST["id"].".html";

 ...
 * 公共英语2级
 * */
class install_yf_gjjsjdj3wljs extends wls {
	...
	public function readList(){
		$content = file("http://www.yfzxmn.cn/com/left/left.jsp?so_id=111&su_id=3");  
		$content = implode("\n", $content);
		$content = mb_convert_encoding($content,'UTF-8','GBK');
		$content = str_replace("\n","",$content);
		
		$fileName="file/yf/国家计算机等级3网络技术.list";
		...
		$fileName="file/yf/国家计算机等级3网络技术.downlist";
		$fileName = mb_convert_encoding($fileName,'GBK','UTF-8');
		$handle=fopen($fileName,"a");
		$ids = '';
		for($i=1;$i<count($arr2);$i++){
			...
			$ids .= $arr3[0].",";		
			fwrite($handle,"http://www.yfzxmn.cn/user/exam/examcontext.jsp?su_id=2&ex_id=".$arr3[0]."\n");
		}
		$ids = substr($ids,0,strlen($ids)-1);
		fclose($handle);
		
		$fileName="file/yf/国家计算机等级3网络技术.ids";
		$fileName = mb_convert_encoding($fileName,'GBK','UTF-8');
		$handle=fopen($fileName,"a");
		fwrite($handle,$ids);
		fclose($handle);		
...
var readPaper = function(){
	...
	$.ajax({
		url: \"wls.php?controller=install_yf_gjjsjdj3wljs&action=downLoadPaper2&id=\"+arr[index],
...
<div id=\"console\"><div>
	...
	public function downLoadPaper2(){
		$fileName="file/yf/计算机类/模拟题试卷/国家计算机等级考试/三级/网络技术/3_".$_REQUEST['id'].".html";
		$fileName = mb_convert_encoding($fileName,'GBK','UTF-8');
		if(file_exists($fileName)){
			return;
		}else{
			$data=file_get_contents("http://www.yfzxmn.cn/user/exam/examcontext.jsp?su_id=3&ex_id=".$_REQUEST['id']);
//			$data = mb_convert_encoding($data,'UTF-8','GBK');
	...
	public function readContent($path=null){
		...
		if($path==null)$path = "E:/Projects/WEBS/PHP/Discuz_7_2/upload/plugins/wls/file/yf/计算机类/模拟题试卷/国家计算机等级考试/三级/网络技术/3_".$_REQUEST["id"].".html";

 ...
 * 公共英语5级
 * */
class install_yf_ggyy5j extends wls {
...
//  http://localhost:8072/wls.php?controller=install_yf_ggyy5j&action=readList	
	public function readList(){
		$content = file("http://www.yfzxmn.cn/com/left/left.jsp?so_id=17&su_id=2");  
		$content = implode("\n", $content);
		$content = mb_convert_encoding($content,'UTF-8','GBK');
		$content = str_replace("\n","",$content);
		
		$fileName="file/yf/英语_公共英语5级.list";
		...
		$fileName="file/yf/英语_公共英语5级.downlist";
		$fileName = mb_convert_encoding($fileName,'GBK','UTF-8');
		$handle=fopen($fileName,"a");
		$ids = '';
		...
		$fileName="file/yf/英语_公共英语5级.ids";
		$fileName = mb_convert_encoding($fileName,'GBK','UTF-8');
		$handle=fopen($fileName,"a");
		fwrite($handle,$ids);
		fclose($handle);		
...
var readPaper = function(){
	...
	$.ajax({
		url: \"wls.php?controller=install_yf_ggyy5j&action=downLoadPaper2&id=\"+arr[index],
...
<div id=\"console\"><div>
	...
	public function downLoadPaper2(){
		$fileName="file/yf/语言类/模拟题试卷/公共英语/公共英语5级/2_".$_REQUEST['id'].".html";
		$fileName = mb_convert_encoding($fileName,'GBK','UTF-8');
		if(file_exists($fileName)){
			return;
		}else{
			$data=file_get_contents("http://www.yfzxmn.cn/user/exam/examcontext.jsp?su_id=2&ex_id=".$_REQUEST['id']);
//			$data = mb_convert_encoding($data,'UTF-8','GBK');
	...
	public function readContent($path=null){
		...
		if($path==null)$path = "file/yf/语言类/模拟题试卷/公共英语/公共英语5级/2_".$_REQUEST["id"].".html";

 ...
 * 公共英语2级
 * */
class install_yf_zjkjkjsw extends wls {
	...
	public function readList(){
		...
		$fileName="file/yf/会计_中级_会计实务.list";
		...
		$fileName="file/yf/会计_中级_会计实务.downlist";
		$fileName = mb_convert_encoding($fileName,'GBK','UTF-8');
		$handle=fopen($fileName,"a");
		$ids = '';
		...
		$fileName="file/yf/会计_中级_会计实务.ids";
		$fileName = mb_convert_encoding($fileName,'GBK','UTF-8');
		$handle=fopen($fileName,"a");
		fwrite($handle,$ids);
		fclose($handle);		
...
var readPaper = function(){
	...
	$.ajax({
		url: \"wls.php?controller=install_yf_zjkjkjsw&action=downLoadPaper2&id=\"+arr[index],
...
<div id=\"console\"><div>
	...
	public function downLoadPaper2(){
		$fileName="file/yf/经济类/模拟题试卷/中级会计/会计实务/6_".$_REQUEST['id'].".html";
		$fileName = mb_convert_encoding($fileName,'GBK','UTF-8');
...
//			$data = mb_convert_encoding($data,'UTF-8','GBK');
	...
	public function readContent($path=null){
		...
		if($path==null)$path = "E:/Projects/WEBS/PHP/Discuz_7_2/upload/plugins/wls/file/yf/经济类/模拟题试卷/中级会计/会计实务/6_".$_REQUEST["id"].".html";

...
<!DOCTYPE root [   <!ELEMENT root ANY>   <!ELEMENT item ANY>   <!ATTLIST item ID ID #REQUIRED>   <!ATTLIST item Explanation CDATA #IMPLIED>]><root>
<item ID="APPPATH">E:\eclipse_workspace\jee\myapp\WebContent\</item>
<item ID="IL8N">zh-cn</item>
<item ID="DB_NAME">wls</item>
<item ID="DB_UNM">root</item>
<item ID="DB_PWD"></item>
<item ID="DB_HOST">localhost:3306:3306</item>
<item ID="MODE">WLS</item>
<item ID="DB_URL" Explanation="JAVA服务端所用的连接参数">jdbc:mysql://localhost:3306/db_4?characterEncoding=utf8</item>
...
select user_type,group_code,permissions from basic_user_session where user_code = __user_code__ and
(
md5( concat( session, hour(now()) ) ) = __session__ or
md5( concat( session, hour(now())-1 ) ) = __session__
)
...
EXISTS (select basic_group.code from basic_group where basic_group.code = basic_user.group_code and basic_group.status = '10')
...
where basic_user.username = __username__ GROUP BY basic_permission.code ORDER BY basic_permission.code ASC
</item>
...
,lastlogintime
,lastlogouttime
,(select name from basic_group where code = basic_user.group_code ) as  group_name
,group_code
,type
,id
,status
...
,creater_code
,creater_group_code,
(select extend4 from basic_memory where code = basic_user.type and extend5 = 'basic_user__type') as type_,
(select extend4 from basic_memory where code = basic_user.status and extend5 = 'basic_user__status') as status_
...
FROM
basic_user
Left Join basic_user_session ON basic_user.username = basic_user_session.user_code
...
FROM
basic_group
Left Join (select user_code,group_code from basic_group_2_user where user_code = __username__) t ON basic_group.code = t.group_code
...
FROM
basic_permission
left Join ( select * from basic_group_2_permission where group_code = __group_code__  ) basic_group_2_permission ON basic_permission.code = basic_group_2_permission.permission_code
...
type,id,status,
(select extend4 from basic_memory where code = basic_group.type and extend5 = 'basic_group__type') as type_,
(select extend4 from basic_memory where code = basic_group.status and extend5 = 'basic_group__status') as status_
...
update basic_memory set extend1 = (select max(id) from exam_paper_multionline) where code = 'exam_paper_multionline' and type = 2 and (select count(id) from exam_paper_multionline) > 0;
...
insert into basic_memory (code,type,extend1) values ('exam_subject_2_user_log_statistics_time','2',0);
update basic_memory set extend1 = (select max(id) from exam_subject_2_user_log_statistics_time) where code = 'exam_subject_2_user_log_statistics_time' and type = 2 and (select count(id) from exam_subject_2_user_log_statistics_time) > 0;
select 1
</item>
...
basic_group
left join
(select * from exam_subject_2_group where subject_code = __code__  ) t on t.group_code = basic_group.code
order by code
</item>
</item>
...
exam_paper_log.mycent_subjective,
exam_paper_log.mycent_objective,
exam_paper_log.count_right,
exam_paper_log.count_wrong,
exam_paper_log.count_giveup,
exam_paper_log.proportion,
exam_paper_log.id,
...
left(exam_paper_log.time_created,10) as time_created,
...
FROM
exam_question_log
Left Join exam_question ON exam_question_log.question_id = exam_question.id
WHERE
...
exam_paper_multionline
Left Join exam_paper ON exam_paper_multionline.paper_id = exam_paper.id
Left Join (  select * from exam_paper_log where exam_paper_log.creater_code = __creater_code__  ) exam_paper_log   ON exam_paper_log.paper_id = exam_paper_multionline.paper_id
</item>
...
select avg(exam_subject_2_user_log.proportion) as data, left(exam_subject_2_user_log.time_created,__gap__) as time,count(*) as count from
exam_subject_2_user_log
where
exam_subject_2_user_log.creater_code = __creater_code__ and exam_subject_2_user_log.subject_code = __subject_code__
...
,(select name from exam_subject where code = exam_subject_2_user_log.subject_code ) as subject_name
from exam_subject_2_user_log where
left(exam_subject_2_user_log.time_created ,__gap__) = left(__date__,__gap__)

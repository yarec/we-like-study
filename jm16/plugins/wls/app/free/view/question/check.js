/**
 * Question Type : Check
 * Compare to quetion-singleChoice , It has no options , only question-title and answer.
 * And two options will be added : right and wrong.
 * */
wls.question.check = Ext.extend(wls.question, {
	
	/**
	 * Add dom to the browser document. 
	 * After the dom added , the questionData will be cleared.
	 * The questionData is setted in wls.question
	 * */
	initDom:function(){
	...
	/**
	 * Add dom to browser document.
	 * There is a casecading button to animate the title.
	 * The total count of quiz's right , wrong and giveup will be cumulatived.
	 * The answerData is setted in wls.question
	 * */
	,showDescription:function(){		
		var obj = this.answerData;		
		this.quiz.cent += parseFloat(obj.cent);
		this.quiz.count.total ++;
		$(".w_qw_tool",$('#w_qs_'+this.id)).append("<a href='#' class='w_q_d_t' title='"+il8n.SeeQuestionAbout+"' id='w_qs_d_t_"+this.id+"' onclick='wls_question_toogle("+this.id+")'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>");
		$('#w_qs_'+this.id).append("<div class='w_q_d'>"+obj.description+"</div>");
		$(".w_q_d_t",$('#w_qs_'+this.id)).addClass("w_q_d_t_2");

		if(obj.myAnswer=='I_DONT_KNOW'){//Give Up
			...
			$('#w_q_subQuesNav_'+this.id).attr('title',il8n.GiveUp+':'+(this.cent));
			wls_question_toogle(this.id);
		}else if(obj.answer==obj.myAnswer){//Right			
			...
			$('#w_q_subQuesNav_'+this.id).attr('title',il8n.Wrong+':'+(this.cent));
	...
	/**
	 * This acted befor the quiz was submitted.
	 * To avoid the student submit the paper without doing anything.
	 * */
	 ...
	 * TODO There is something wrong between ExtJS and Jquery.
	 * */
	,setMyAnser:function(){

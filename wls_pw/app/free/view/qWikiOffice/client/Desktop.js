/*
 * qWikiOffice Desktop 1.0 Copyright(c) 2007-2010, Murdock Technologies, Inc.
...
Ext.Desktop = Ext.extend(Ext.util.Observable, {
			...
			startMenuSortFn : function(a, b) { // Sort in ASC alphabetical
												// order with menus at the top
				if (((b.menu && a.menu) && (b.text < a.text))
						|| (b.menu && !a.menu)
						|| ((b.text < a.text) && !a.menu)) {
					return true;
				}
				return false;
			},
			...
			constructor : function(config) {
				...
				this.contextMenu = new Ext.menu.Menu({
							items : [{
									...
									/*
									 * { text: '切换背景', handler: function(){
									 * 
									 * var mod = parseInt(Math.random()*6);
									 * 
									 * var o = { id: mod, name: 'Blank', file:
									 * 'free/view/qWikiOffice/resources/wallpapers/wallpaper'+mod+'.jpg' };
									 * QoDesk.App.getDesktop().setWallpaper(o); },
									 * scope: this },
									 */
									'-', {
										text : '窗口全关',
							...
							/*
							 * { text: 'Tile', handler: this.tileWindows, scope:
							 * this }, { text: 'Cascade', handler:
							 * this.cascadeWindows, scope: this }, { text:
							 * 'Checkerboard', handler:
							 * this.checkerboardWindows, scope: this }, { text:
							 * 'Snap Fit', handler: this.snapFitWindows, scope:
							 * this }, { text: 'Snap Fit Vertical', handler:
							 * this.snapFitVWindows, scope: this }, '-'
							 */
							]
						});
			...
			initModules : function() {
				...
				for (var i = 0, len = ms.length; i < len; i++) {
					if (ms[i].launcher) {
						...
						ms[i].launcher.handler = this.launchWindow
								.createDelegate(this, [ms[i].id]);
					}
					ms[i].loaded = false;
				}
				...
				if (ms) {
					for (var i = 0, len = ms.length; i < len; i++) {
						if (ms[i].launcherPaths
								&& ms[i].launcherPaths.contextmenu) {
			...
			getWinY : function(height) {
				return (Ext.lib.Dom.getViewHeight() - this.getTaskbarHeight() - height)
						/ 2;
			},
			...
			setTaskbarTransparency : function(v) {
				if (v >= 0 && v <= 100) {
					...
					Ext.util.CSS.updateRule('.transparent', '-moz-opacity', v
									/ 100);
			...
			setWallpaper : function(o) {
				...
				function verify() {
					if (wp.complete) {
						...
						notifyWin.setMessage('背景图导入' + o.id);
...
(function			() {
			...
			addContextMenuItem : function(id) {
				...
				if (m && !m.contextMenuItem && m.launcher) {
					...
					this.contextMenu.add({
								handler : this.launchWindow.createDelegate(
										this, [id]),
								iconCls : c.iconCls,
								text : c.text,
								tooltip : c.tooltip || ''
			...
			addShortcut : function(id, updateConfig) {
				...
				if (m && !m.shortcut) {
					...
					$("."+c.shortcutIconCls).css("width","74");
				}
			...
			buildStartItemConfig : function() {
				...
				if (ms) {
					...
					for (var i = 0, iLen = ms.length; i < iLen; i++) { // loop
																		// through
																		// the
																		// modules
						if (ms[i].launcherPaths) {
							...
							for (var id in launcherPaths) { // loop through the
															// module's launcher
															// paths
								...
								if (paths.length > 0) {
									if (id === 'startmenu') {
										simplify(smi[0].menu, paths,
												ms[i].launcher);
										sort(smi[0].menu);
									} else if (id === 'startmenutool') {
										simplify(smi[1].menu, paths,
												ms[i].launcher);
										sort(smi[1].menu);
									}
								}
				 ...
				 * 
				 * @param {array}
				 *            pMenu The Start Menu
				 * @param {array}
				 *            paths The menu texts
				 * @param {object}
				 *            launcher The launcher config
				 */
				function simplify(pMenu, paths, launcher) {
					...
					for (var i = 0, len = paths.length; i < len; i++) {
						...
						foundMenu = findMenu(pMenu.items, paths[i]); // text
																		// exists?
				 ...
				 * 
				 * @param {array}
				 *            pMenu The parent menu to search
				 * @param {string}
				 *            text
				 */
				...
				/**
				 * @param {array}
				 *            menu The nested array to sort
				 */
				function sort(menu) {
					var items = menu.items;
					for (var i = 0, ilen = items.length; i < ilen; i++) {
						if (items[i].menu) {
							sort(items[i].menu); // use recursion to iterate
													// nested arrays
						}
						bubbleSort(items, 0, items.length); // sort the menu
															// items
					}
				}

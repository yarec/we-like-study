...
class subject extends wls{
	...
	public function getComboList(){
		$data = $this->m->getList(1,100);
		$data = $data['data'];
		//print_r($data);exit();
		$data2 = array();
		for($i=0;$i<count($data);$i++){
			if($data[$i]['isleaf']==1)$data2[] = array($data[$i]['id_level'],$data[$i]['name']);
		}
...
$xml .= '		</graph>
	...
	public function add(){
		...
		//The admin group should have the access to every subject.
		//So once new subject added , must set it to admin group, admin group's level_id is 10
		//Then clean the user cache.
		$obj = new m_user_group();
		$obj->linkSubject(array(
			 'id_level_subject'=>$_POST['id_level']
			,'id_level_group'=>'10'
		));

...
class m_quiz_paper_yf_gajczs extends m_quiz_paper_yf implements yfActions{
	...
	public function getChoiceQuestions(){
		$content = $this->paperHtmlContent;
		$p1 = strpos($content,"二、单项选择题");
		$p2 = strpos($content,"三、多项选择题");		
		$content = substr($content,$p1,$p2-$p1);	
		
		for($i=1;$i<=30;$i++){
			$p1 = strpos($content,">".$i.".</td>");
			$p2 = strpos($content,"name=s".($i+30-1)."fs");
			$data = substr($content,$p1,$p2-$p1);
			...
			$p1 = strpos($data,"name=s".($i+30-1)."an");
			$p2 = strpos($data,"name=s".($i+30-1)."t");
			$answer = substr($data,$p1,$p2-$p1);
			$answer = str_replace("name=s".($i+30-1)."an type=hidden value=\"","",$answer);
			$answer = str_replace("\"><input","",$answer);
			$answer = trim($answer);
	...
	public function getMultiChoiceQuestions(){
		$content = $this->paperHtmlContent;
		$p1 = strpos($content,"三、多项选择题");
		$p2 = strlen($content);	
		$content = substr($content,$p1,$p2-$p1);	
		
		for($i=1;$i<=20;$i++){
			$p1 = strpos($content,">".$i.".</td>");
			$p2 = strpos($content,"name=s".($i+60-1)."fs");
			$data = substr($content,$p1,$p2-$p1);
			$data = str_replace("<td width=\"50%\">","",$data);
			$data = str_replace("</td>","",$data);
			$data = str_replace("<tr>","",$data);
			...
			$p1 = strpos($data,"name=s".($i+60-1)."an");
			$p2 = strpos($data,"name=s".($i+60-1)."t");
			$answer = substr($data,$p1,$p2-$p1);
			$answer = str_replace("name=s".($i+60-1)."an type=hidden value=\"","",$answer);
			$answer = str_replace("\"><input","",$answer);
			$answer = trim($answer);
			$answerArr = array();

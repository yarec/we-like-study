...
Ext.extend(Ext.app.App, Ext.util.Observable, {
			 ...
			 * 
			 * Ext.Ajax.request({ url: this.app.connection, // Could also pass
			 * the module id in the querystring like this. // url:
			...
			loadModule : function(m) {
				...
				Ext.Ajax.request({
							...
							success : function(o) {
								...
								notifyWin.setTitle(il8n.success);// TODO
								notifyWin.setMessage(moduleName + ' '+ il8n.importFile);
			...
			loadModuleComplete : function(success, id) {
				if (success === true && id) {
					...
					if (m) {
						...
						m.loaded = true;
						m.init();
						m.on('actioncomplete', this.onModuleActionComplete,
								this);
				...
				if (p && p.loaded === false) {
					if (eval('typeof ' + p.className) === 'function') {
						...
						for (var i = 0, len = ms.length; i < len; i++) { // replace
																			...
																			// module
							if (ms[i].id === m.id) {
								Ext.apply(m, ms[i]); // transfer launcher
														// properties
								ms[i] = m;
							}
						}
			...
			registerModule : function(m) {
				...
				this.modules.push(m);
				m.launcher.handler = this.desktop.launchWindow.createDelegate(
						this.desktop, [m.id]);
				m.app = this;
				return true;
			},
			...
			isAllowedTo : function(method, id) {
				if (method !== '' && id != '') {
					...
					if (p[id] && Ext.isArray(p[id])
							&& p[id].indexOf(method) !== -1) {
						return true;
					}
				}
				return false;
			...
			onModuleActionComplete : function(module, data, options) {
				this.fireEvent('moduleactioncomplete', this, module, data,
						options);
			}
		});